...
function NavegacaoPaginacao({ pagina, totalPaginas, tipo, busca, inicial }: PropsPaginacao) {
  ...
  return (
    <nav className="mt-16 flex items-center justify-center gap-2" aria-label="Paginação">
      {pagina > 1 && (
        <Link href={href(pagina - 1)} className="w-10 h-10 border border-outline-variant flex items-center justify-center text-on-surface-variant hover:border-primary hover:text-primary transition-all">
          <ChevronLeft size={20} />
        </Link>
      )}
      {pages.map((p) => (
  ...
  return (
    <div className="max-w-4xl mx-auto px-margin-mobile md:px-6 py-12 md:py-24">
      ...
      <div className="flex items-center justify-center w-full my-12">
        ...
        <span className="px-4 text-secondary"><Sparkles size={20} /></span>
        <div className="flex-1 h-px bg-secondary/30" />
      </div>
